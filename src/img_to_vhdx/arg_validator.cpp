 ...
 *
 * This function checks if the filename ends with either ".mrimg" or ".mrbak".
...
bool isValidExtension(const std::wstring& filename) {
	if (filename.length() < 6) { // Minimum length to hold either extension
		return false;
	}

	std::wstring extension = filename.substr(filename.length() - 6);
	std::transform(extension.begin(), extension.end(), extension.begin(), ::tolower);

	return extension == L".mrimg" || extension == L".mrbak";
}
...
std::tuple<std::wstring, std::wstring, std::wstring, int, bool, bool, bool> parseCommandLineParameters(int argc, wchar_t* argv[])
{
    ...
    if (!isValidExtension(filename)) {
        throw std::invalid_argument("Invalid file extension. Only .mrimg and .mrbak are allowed.");
    }
