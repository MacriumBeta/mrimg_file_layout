...
std::wstring prepareVhdxFileName(const std::wstring& filename, const std::wstring& outputPath) {
    ...
    // Find the position of ".mrimg" in vhdxName
    std::wstring::size_type index = vhdxName.rfind(L".mrimg");

    // If ".mrimg" is found, replace it with ".vhdx"
    if (index != std::wstring::npos) {
        vhdxName.replace(index, 6, L".vhdx");
    }
    else {
        // If ".mrimg" is not found, find the position of ".mrbak" in vhdxName
        index = vhdxName.rfind(L".mrbak");
        // If ".mrbak" is found, replace it with ".vhdx"
        if (index != std::wstring::npos) {
            vhdxName.replace(index, 6, L".vhdx");
        }
    }
